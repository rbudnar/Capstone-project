...
class TrainingRunner:
	def __init__(self, filename="train_1.csv", cell_model_path=CELL_MODEL_PATH, train_mode=TrainingMode.SINGLE, epochs=EPOCHS, batch_size=BATCH_SIZE, val_split=VAL_SPLIT, use_wandb=False, use_tb=True):
		...
		if train_mode is TrainingMode.MULTI:
			...
			self.build_simple_model(df)

	def summary(self):
		if(self.model):
			self.model.summary()

	# def setup_callbacks(self, checkpoint_path="saved_models/weights.best.from_scratch.hdf5"): 
	...
	def create_multi_model(self, use_cell_model=False, cell_model_path=CELL_MODEL_PATH):
		...
		self.train_generator = MultiGenerator(train_x, train_y, batch_size, is_train=True)
		self.valid_generator = MultiGenerator(valid_x, valid_y, batch_size)
	...
	def run(self):
		...
		if self.train_mode is TrainingMode.CELL_ONLY:
			...
			checkpoint_path = 'saved_models/weights.{self.time_stamp}.hdf5'
